...
from main import (
    ...
    _parse_iso_datetime,
    _report_category,
    _report_post_branch_name,
    _report_post_content,
    _report_post_pr_body,
    _report_post_relative_path,
    _report_summary,
...
class PipelineHelperTests(unittest.TestCase):
    ...
    def test_report_post_relative_path_matches_jekyll_structure(self):
        created_at = datetime(2026, 1, 18, 11, 0, tzinfo=UTC)
        self.assertEqual(
            _report_post_relative_path("Report Title", created_at),
            "_posts/2026/01/2026-01-18-report-title.md",
        )

    def test_report_post_branch_name_uses_timestamped_pr_branch(self):
        created_at = datetime(2026, 1, 18, 11, 0, tzinfo=UTC)
        self.assertEqual(
            _report_post_branch_name("Report Title", created_at),
            "report-post/20260118-110000-report-title",
        )

    def test_report_summary_extracts_first_meaningful_paragraph(self):
        body = "# Heading\n\nThis is the lead paragraph with enough detail to act as the summary.\n\n## Section\n\nMore detail here."
        self.assertEqual(
            _report_summary(body),
            "This is the lead paragraph with enough detail to act as the summary.",
        )

    def test_report_category_detects_league_keywords(self):
        self.assertEqual(
            _report_category("Arsenal and Liverpool tactical shift", "A Premier League tactical story."),
            "Premier League",
        )
        self.assertEqual(
            _report_category("Cross-league recruitment pattern", "No explicit competition match."),
            "General",
        )

    def test_report_post_content_uses_requested_front_matter_shape(self):
        created_at = datetime(2026, 1, 18, 11, 0, tzinfo=UTC)
        content = _report_post_content(
            "Report Title",
        ...
        self.assertIn('title: "Report Title"', content)
        self.assertIn('categories:\n- "Premier League"', content)
        self.assertIn("\n<!---more--->\n", content)
        self.assertTrue(content.endswith("Rest of the report.\n"))

    def test_report_post_pr_body_includes_path_category_and_summary(self):
        created_at = datetime(2026, 1, 18, 11, 0, tzinfo=UTC)
        body = _report_post_pr_body(
            "_posts/2026/01/2026-01-18-report-title.md",
        ...
        self.assertIn("Automated report post generated by the research pipeline.", body)
        self.assertIn("`_posts/2026/01/2026-01-18-report-title.md`", body)
        self.assertIn("- Date: 2026-01-18 11:00 UTC", body)
        self.assertIn("- Category: Premier League", body)
        self.assertTrue(body.endswith("Short report summary."))
